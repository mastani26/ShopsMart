  ...
  return (
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink to='/ \' onClick={()=>{setOpen(false)}}>
                <img className='h-9' src={assets.logo} alt="MyLogo" />
            </NavLink>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {!user ?(
                    ...
                    <div className='relative group'>
                        ...
                        <ul className='hidden group-hover:block absolute top-10 right-0 big-white shawdow
        ...
        { open && (
            <div className={`${open ? 'flex' : 'hidden'} absolute top-15 left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                ...
                {!user ? (<button onClick={() =>{
                    setOpen(false);
                    setshowuserlogin(true);
                }} className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-primary-dull-dull-dull-dull-dull-dull transition text-white rounded-full text-sm">
                    Login
                </button>):(
                    <button onClick={LogOut} className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-primary-dull-dull-dull-dull-dull-dull transition text-white rounded-full text-sm">
                    LogOut
                </button>
                )}
            </div>
